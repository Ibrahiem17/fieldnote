...
- **Web preview may still not open the database, depending on your browser — three layers deep, see `docs/DESIGN.md` D-008/D-009/D-010 for the full trail.** Plain `npm run web` can't satisfy `expo-sqlite`'s `SharedArrayBuffer` requirement at all. `npm run web:coi` fixes that via a small reverse proxy. `expo-sqlite` web also needs an OPFS API (`createSyncAccessHandle`) some browsers lack — patched via `patch-package` (D-009) to degrade gracefully (in-memory only) instead of hanging. Past both of those, this project's own sandboxed dev environment hit a _third_, deeper issue — confirmed by direct instrumentation, not guessed — where the browser's `SharedArrayBuffer` doesn't behave as true shared memory across the worker boundary, which every Drizzle/`expo-sqlite` web query depends on with no fallback. That's a browser-engine threading gap, not fixable from this codebase. It's also old, foundational browser functionality, unlike the newer OPFS API — so `npm run web:coi` has a real chance of working end-to-end in an ordinary up-to-date desktop Chrome or Edge, just untested here. None of this touches Android or iOS — both use native on-device SQLite and were never affected. Web was never a target platform for this project.
