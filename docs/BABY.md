...
### `src/lib/syncTriggers.ts` — checking for waiting work (D-045)
`const PERIODIC_CHECK_MS = 15_000;` — 15 seconds (`15_000` is just `15000` with a readability underscore). `let connectedNow = false;` is updated whenever the network listener fires, so the timer knows whether the phone is online. `setInterval(async () => { ... }, PERIODIC_CHECK_MS)` runs the function every 15 seconds; inside, `if (!connectedNow) return;` skips when offline, otherwise `const { pending } = await getOutboxSummary(); if (pending > 0) void triggerSync("periodic");` — anything waiting? then run a normal sync (`void` = start it, don't wait). The function returned at the end now also calls `clearInterval(periodic)` so the timer stops when signed out.

### `src/lib/uploadStatus.ts` and `src/components/UploadBanner.tsx`
`describeUpload(status)` returns `{ text, tone }` or `null`. `n === 1 ? "change" : "changes"` picks singular or plural. `if (status.deadLettered > 0) return ...` comes first so a problem always outranks "still uploading". In the banner, `const net = useNetInfo();` is a hook that re-renders the component whenever the connection changes; `Boolean(net.isConnected && net.isInternetReachable !== false)` means "connected, and not known to be cut off". `if (!message) return null;` — a component that returns `null` draws nothing. `setInterval(refresh, POLL_MS)` re-reads the queue every 4 seconds; the cleanup function (`return () => { ... clearInterval(timer); }`) stops it when the screen goes away.
...
### `scripts/make-brand-assets.mjs` (D-047)
`import sharp from "sharp"` — a library that turns SVG drawings into PNG pictures. `const dome = "M262 610 C262 400 ... Z"` — an SVG *path*: `M` moves the pen, `C` draws a smooth curve (two guide points and an end point), `Z` closes the shape; that is the helmet's rounded top. `art()` returns the drawing as text: an ellipse for the shadow, the dome, a `clipPath` (a stencil) so the amber band and the purple ridge only show inside the dome, a rounded rectangle for the brim, and a circle with a `stroke` (outline) plus a thick round-capped `path` for the check. `translate(-15 -39)` nudges the drawing so its visual middle sits at the picture's centre. `scaled(inner, s)` shrinks or grows a drawing around the centre (`translate(512 512) scale(s) translate(-512 -512)`). `art({ mono: true })` draws one flat colour and uses a `mask` (black areas are cut out) to leave gaps for the ridge, the band and around the badge. `gradient` defines a diagonal purple gradient and a soft white "gloss". The five `files` are the finished SVGs: the icon (art scaled up), the Android *foreground* (art at 74% so the phone's icon mask can't cut it), the *background* (gradient only), the *monochrome* one, and the *splash* (a rounded purple tile with the art on it). `sharp(Buffer.from(svg), { density: 300 }).resize(size, size).png().toFile(...)` renders each one; `await` waits for it to finish.

### `src/app/_layout.tsx` — splash (D-047)
`SplashScreen.preventAutoHideAsync()` — tell the phone "keep the splash showing until I say so"; `.catch(() => {})` ignores an error if it was already hidden. `setOptions({ duration: 350, fade: true })` — fade out over 0.35 s. `setTimeout(() => SplashScreen.hideAsync(), 8000)` — a safety net: after 8 seconds hide it no matter what. In `AuthGate`, `useEffect(() => { if (!loading) SplashScreen.hideAsync() }, [loading])` runs whenever `loading` changes and hides the splash the moment the sign-in check finishes (`AuthGate` only renders after the fonts are loaded, so both conditions are met).

### `src/lib/guideSeen.ts`
`Storage from "expo-sqlite/kv-store"` — a tiny built-in "key → text" store (like a note-to-self drawer). `getItem(KEY)` reads it, `setItem(KEY, "1")` writes it. Both are wrapped in `try/catch`; on a read failure `hasSeenGuide` returns `true` (pretend it was seen) so the guide is never forced on someone.

### `src/app/(tabs)/_layout.tsx` — auto-open
`usePathname()` gives the current address (`"/"` is the home screen). The effect runs once (`[]`): `if (pathname !== "/") return;` skips deep links; `if (await hasSeenGuide()) return;` skips if seen; otherwise `await markGuideSeen()` then `router.navigate("/guide")`. `cancelled` guards against the screen going away while waiting.

### `src/lib/guideContent.ts`, `guide.tsx`
`GUIDE` is one big object of text. `satisfies GuideStep[]` makes TypeScript check every step has an icon, title and body without widening the type. `BANNED_GUIDE_WORDS` feeds the test that fails if a developer word slips in. `guide.tsx` maps over `GUIDE.steps.items`; `STEP_TONES[i % STEP_TONES.length]` picks a card colour in turn; `cardColors[tone].on` is the right text colour for that card, used to tint the icon circle (`rgba(...)` = red, green, blue, opacity); `enterIndex={i + 2}` staggers each card's entrance after the ones above it.
