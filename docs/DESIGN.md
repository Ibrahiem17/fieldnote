...
- **What this does and doesn't fix, being honest about it:** the proxy genuinely solves the `SharedArrayBuffer` problem — confirmed directly (`crossOriginIsolated: true`, `typeof SharedArrayBuffer === "function"`) after switching to it. It does **not** make `expo-sqlite` web fully work in every browser: `expo-sqlite`'s persistent storage VFS (`AccessHandlePoolVFS`) unconditionally calls `FileSystemFileHandle.createSyncAccessHandle()` at startup, an OPFS API that is not implemented in every Chromium build — confirmed directly by testing that exact call, which returned `"createSyncAccessHandle is not a function"` in this project's sandboxed preview browser. That is a real gap in that specific browser's engine, not something a proxy, `metro.config.js`, or this app's own code can work around; it would very plausibly work in an up-to-date desktop Chrome or Edge, which do implement that API. Nothing about this affects Android or iOS, which use native on-device SQLite and never touch any of this code path.
...
- **Rejected:** patching `expo-sqlite`'s compiled `node_modules` code directly (e.g. via `patch-package`) to skip the eager `AccessHandlePoolVFS` initialization — technically possible, but fragile against every `expo-sqlite` upgrade, and disproportionate effort to fully support a platform this project was never shipping to.
