...
## Answers that never left the phone

When you type into a form, the app saves each answer on the phone straight away and also adds a note to a "to send" list. Type again a moment later and the app tidies the list: instead of two notes for the same answer, it keeps one with the newest text. That tidying had a flaw. The first note said "create this answer". The tidy-up threw it away and wrote "change this answer" instead — but the office can't change something it never received. So the change did nothing, and the answer stayed on the phone forever, while everything looked fine on screen. Any answer you edited more than once before the phone got a signal was affected — which, for typed text, is nearly all of them. The fix: while the office still hasn't received the original, the tidy-up keeps it as "create this answer" and just puts the newest text inside it.

**Smaller fixes in the same pass.** A number box used to lose the decimal point as you typed (so 3.3 was impossible, and emptying the box put a 0 in it); it now keeps exactly what you type. There's a new "date" question type — you type the digits and the dashes appear by themselves, and the app refuses dates that don't exist, like 30 February. And a question can now appear only when another answer equals a particular value, or when another question has any answer at all.

## Being offline isn't the same as being refused

When the phone tries to send its queued work and something goes wrong, there are two very different situations. Sometimes the office answers "no" — the request was wrong, or you're not allowed. Retrying that a few times, then setting it aside for a person to look at, is sensible. But sometimes there's simply no answer at all, because you're in a basement with no signal. That says nothing about your data.

The app used to treat both the same. Each time you opened it without signal, it tried, failed, and counted a strike against every waiting item; after eight strikes — a few minutes of using the app — it set them all aside as "failed" and waited for you to press Retry. Someone working a full day without coverage would come back to a screen of failures that were never actually failures. Now, "couldn't reach the office" costs nothing: the items stay exactly as they were, the app stops trying as soon as it sees there's no signal (no point knocking on twenty doors when the road is closed), and everything is sent, once, when the signal returns. Photos had a smaller version of the same trap: a signal dropping at the wrong moment made the app announce "you're not signed in" and give up on the photo for good. It now simply reads who's signed in from the phone's own memory.

This is also the first time the sending machinery itself has automated tests: success, order (a project before things inside it), backing off after a refusal, giving up after too many, the Retry button, and the offline behaviour above.

## Why it felt hard to use — and what "usable" means here

The app worked, but it behaved like a tool built by the person who understands it. Buttons that did something slow gave no sign anything was happening — tap "Add photo" and nothing changed on screen for several seconds while the camera loaded, so a newcomer taps again and decides it's broken. Actions that succeeded finished in silence. Some words came from the inside of the machine ("outbox rows", "pending", "template"). The screen had a "Save Changes" button even though most of it saved by itself, so nobody could tell what was already safe. Marking an inspection finished failed with a list like `roof_condition: Required`, which is the machine's name for the question, not the one on the screen. A location appeared as a line of code, and a signature as a file path.

The fixes all follow one idea: **say what's happening, in the person's words, at the moment it happens.** A button shows a spinner and "Opening camera…" the instant it's tapped. A small green message says "Photo added". A line under the form says "All changes saved ✓". Finishing an inspection points at the exact questions that still need an answer, in red, under each one. A location reads "31.4646° N, 74.2397° E" with a button to see it on a map; a signature shows as a picture. Settings says "2 waiting to upload" and "Everything is uploaded ✓". And the sign-in screen tells the truth: you need internet the first time, after that it works without signal.

Nothing here changes how data is stored or sent — it's all about a stranger being able to tell what the app is doing and what to do next.

## Small things that decide whether a stranger gives up

A form that doesn't say which questions you *must* answer makes you find out by failing. Now the questions you must answer carry a star, and a note at the top says what the star means. A date is annoying to type on a phone, so there's a "Use today's date" button. A project with a typo in its name used to be stuck forever; now you can edit it, and you can delete it — but because a project's inspections live inside it, deleting one also deletes those inspections, so the app tells you how many before you agree (and, like everything else, the deletion is sent to your account when there's signal). And the small status under each item now says what a person would say — "Waiting to upload", "Uploaded ✓" — instead of "Pending" and "Synced".

## "It uploads by itself" — now it really does
