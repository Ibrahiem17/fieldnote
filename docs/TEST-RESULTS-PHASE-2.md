...
## Correction — 2026-09-13 fix pass

The Phase 2 merge this file originally described did not actually pass `npm run typecheck`/`npm run lint` — `npm install` itself failed (a declared package version didn't exist). Full details and every finding: `docs/DESIGN.md` D-016. The short version, since it changes how the checklist below should be read:

- Now genuinely true after fixing: `npm install`, `npm run typecheck`, `npm run lint` all pass clean.
- The **conditional visibility** line below was checked off as done, but wasn't — `visibleIf` was checked during validation only; nothing filtered it out of what actually rendered, so a conditionally-hidden field showed unconditionally on every inspection. Confirmed broken, then confirmed fixed, live in the running web preview (not assumed from the code alone). See D-016 for exactly how it was verified.
- Everything already marked "not device-verified" below is still not device-verified. Fixing the build and one real rendering bug is not device testing — that gap is exactly as open as it was before this pass.
...
- [x] Conditional visibility shows and hides correctly (hidden answers preserved, excluded from validation) — _was checked prematurely; was actually broken (rendered unconditionally), fixed and live-verified 2026-09-13, see correction above_
